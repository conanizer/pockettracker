...
- **Device-tested & working:** MP3, FLAC, M4A, OGG-Vorbis, **Opus** (load/preview/pitch/save/reload/render).
- **Opus — bundled natively (libopus), device-tested.** MediaCodec couldn't decode Opus on the target
  ROMs (LineageOS *and* GammaOS both errored at codec start), so Opus is bundled natively (`.opus` +
  Opus-in-`.ogg`). A `.opus` file loads and plays (`✅ Native-decoded opus`). ~+1.5–2 MB APK; no
  runtime cost (one-shot decode at load, outside the audio path). (The recurring failing test file was
  a red herring — an iPhone **ALAC** voice memo mislabeled `.ogg`, never Opus.)
