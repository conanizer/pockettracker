...
- ✅ `effects/primitives/daisysp/` — vendored DaisySP `Svf` + `dsp.h` (MIT licence)
- ✅ `effects/modules/filter-module.h` — `FilterModule` (LP/HP/BP via `daisysp::Svf`; `setParams()` + `processMono/Stereo`)
...
- `OTTModule` (composed from `daisysp::Svf` crossovers + EnvelopeFollower)
